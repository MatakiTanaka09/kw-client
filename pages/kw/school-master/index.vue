    ...
    <div class="school__master__container">
        <div class="school__master__wrapper">
            ...
            <div class="create__button">
                <nuxt-link :to="{ name: 'kw-school-master-new' }">
        ...
        <table class="table">
            ...
            >
                ...
                <td>{{ res.email }}</td>
                <td>{{ res.tel }}</td>
                <td><a href="">{{ res.url }}</a></td>
                <td><a href="">{{ res.icon }}</a></td>
                <td>{{ formatZipCode(res.zip_code1, res.zip_code2) }}</td>
                <td>{{ formatAddress(res.state, res.city, res.address1, res.address2) }}</td>
                <td class="td-expand">
                    <nuxt-link :to="{ name: 'kw-school-master-id', params: { id: res.id } }">
                        <button class="button">詳細</button>
                    </nuxt-link>
                    <nuxt-link :to="{ name: 'kw-school-master-id-edit', params: { id: res.id } }">
